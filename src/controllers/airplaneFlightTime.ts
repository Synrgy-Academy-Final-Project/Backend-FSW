...
export class AirplaneFlightTimeController {
    ...
    public getAllAirplaneFlightTime = async (
        _: Request,
        res: Response
    ): Promise<Response<any, Record<string, any>> | undefined> => {
        try {
            ...
            if (airplaneFlightTimes.length === 0) {
                return res.status(404).json({
                    status: 404,
                    message: 'Airplane Flight Time is not found!',
                    data: [],
                })
            }
            ...
            res.status(200).json({
                status: 200,
                message: 'Success get all airplane flight time',
                data: airplaneFlightTimes,
            })
        } catch (error: any) {
            console.error(error)
    ...
    public getAirplaneFlightTimeById = async (
        req: Request,
        res: Response
    ): Promise<Response<any, Record<string, any>> | undefined> => {
        try {
            ...
            const airplaneClass = await this.airplaneFlightTimeService.getAirplaneFlightTimeById(id)

            res.status(200).json({
                status: 200,
                message: 'Success get airplane flight time by id',
                data: airplaneClass,
            })
        } catch (error: any) {
            console.error(error)
